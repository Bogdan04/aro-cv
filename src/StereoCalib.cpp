...
int main()
{
    ...
    //---------Starting WebCam----------
        ...
        if(capture1!=NULL) return -1; cvWaitKey(100);
        capture2= cvCaptureFromCAM(1);
        if(capture2!=NULL) return -1;
    ...
    //--------Find chessboard corner--------------------------------------------------
        ...
        //Calib with Bouguet algrithm
            ...
            cvStereoRectify( &_M1, &_M2, &_D1, &_D2, imageSize,&_R, &_T,&_R1, &_R2, &_P1, &_P2, &_Q,0CV_CALIB_ZERO_DISPARITY );
